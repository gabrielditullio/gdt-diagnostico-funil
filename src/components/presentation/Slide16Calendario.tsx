...
const retreatMonths = [0, 2, 4, 6, 8, 10]; // Jan, Mar, Mai, Jul, Set, Nov
      ...
      <div className="grid grid-cols-4 md:grid-cols-6 gap-2 max-w-2xl mx-auto">
        {months.map((m, i) => {
          ...
          return (
            <div
              key={i}
              className={`rounded-lg border p-2.5 text-center space-y-1.5 transition-all ${
                isRetreat
            ...
            >
              <p className="text-[10px] font-bold text-muted-foreground uppercase">{m}</p>
              {isRetreat && (
                <>
                  <Mountain size={16} className="text-primary mx-auto" />
                  <p className="text-[7px] font-bold text-primary">RETIRO</p>
                </>
              )}
              {campaignStart && !isRetreat && (
                <>
                  <Megaphone size={14} className="text-gold mx-auto" />
                  <p className="text-[7px] font-bold text-gold leading-tight">Campanha<br />8 sem.</p>
                </>
              )}
            </div>
          );
          ...
          {cycle.map((step, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <div className="flex items-center gap-1 rounded-md border border-border bg-card px-2.5 py-1.5">
                ...
                <span className="text-[9px] font-semibold text-foreground whitespace-nowrap">{step.label}</span>
              </div>
              {i < cycle.length - 1 && (
                <ArrowRight size={12} className="text-muted-foreground shrink-0" />
              )}
              {i === cycle.length - 1 && (
                <RotateCcw size={12} className="text-primary shrink-0" />
              )}
            </div>
          ))}
        </div>
